...
const Quiz = ({ courseProgress, course, quizSet, isTaken, markFromQuiz }) => {

    const [open, setOpen] = useState(false)
    const totalQuizMark = course.quizSet?.quizIds?.length * 5 ?? 0
        ...
        <div class="max-w-[270px] bg-white border border-border rounded-md dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-4">
                ...
                <p class="mb-4 font-normal text-gray-500 dark:text-gray-400 text-sm">
                    {courseProgress > 100 ? "Before participating in quiz you have to complete the course first!" : "This is in optional but highly recommended.Test you learing improvment by participating quiz"}
                </p>
                <Button
                ...
                >
                    ...
                    <span>{isTaken ? "View Quiz" : "Participate a Quiz"}</span>
                </Button>
            </div>
            <QuizModal courseId={course.id} quizSetId={quizSet._id.toString()} open={open} setOpen={setOpen} quizzes={quizzes} isTaken={isTaken} />
        </div>
    )
}
